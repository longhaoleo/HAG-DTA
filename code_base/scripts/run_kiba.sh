#!/bin/bash
# run_kiba.sh — KIBA, 单 seed（分布式用）
# Usage: bash scripts/run_kiba.sh <seed>
#   seed: 100, 1000, 2000, 3000, 4000

set -e
SEED=${1:?Usage: bash run_kiba.sh <seed>}
cd "$(dirname "$0")/.."

OUTPUT="${HAG_DTA_OUTPUT_ROOT:-/root/autodl-tmp/HAG-DTA-runs}"
mkdir -p "$OUTPUT/logs"

log() { echo "[$(date '+%H:%M:%S')] $*" | tee -a "$OUTPUT/logs/kiba_s${SEED}.log"; }
...
" >> "$OUTPUT/logs/kiba_s${SEED}.log" 2>&1
...
ct.SEEDS = [$SEED]
import sys
sys.argv = ['training_graphdta.py', '1', '0']
...
ct.SEEDS = [$SEED]
import sys
sys.argv = ['training_graphdta.py', '1', '1']
exec(open('training_graphdta.py').read())
" >> "$OUTPUT/logs/kiba_s${SEED}.log" 2>&1
...
ct.SEEDS = [$SEED]
import sys
sys.argv = ['training_graphdta.py', '1', '2']
exec(open('training_graphdta.py').read())
" >> "$OUTPUT/logs/kiba_s${SEED}.log" 2>&1
...
ct.SEEDS = [$SEED]
import sys
sys.argv = ['training_graphdta.py', '1', '3']
exec(open('training_graphdta.py').read())
" >> "$OUTPUT/logs/kiba_s${SEED}.log" 2>&1
