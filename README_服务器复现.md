...
# 1. 安装 PyTorch 2.0.0 GPU 版（必须为 GPU 版，下面是 CUDA 11.8 示例）
pip install torch==2.0.0 torchvision==0.15.0 torchaudio==2.0.0 --index-url https://download.pytorch.org/whl/cu118

# 2. 安装 PyTorch Geometric 和 PyG 扩展（匹配 PyTorch 2.0 + CUDA 11.8）
# 以下版本经测试可用，可直接通过 PyG 官方源安装
...
# 备选：如果网络不稳定或上述命令失败，可使用项目目录下已提供的 Linux wheel 文件
...
> ⚠️ **版本检查**：上述命令安装的版本组合已验证可用：PyTorch 2.0.0+cu118 + torch_geometric 2.6.1 + torch_scatter 2.1.2 + torch_sparse 0.6.18 + torch_cluster 1.6.3 + torch_spline_conv 1.2.2。请确保服务器 CUDA 版本为 11.8，否则需要更换 `cu118` 为对应的 CUDA 版本号（如 cu117、cu121 等）。
