...
for dataset in datasets:
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
        ...
        if dataset == 'kiba':
            ...
            valid_loader1 = DenseDataLoader(valid_data1, batch_size=TRAIN_BATCH_SIZE, shuffle=False)
...
for seed in [100,1000,2000]:# 设置随机种子
    ...
    model = model_select(num_nodes_1=4,num_nodes_2 = 2)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if dataset=='kiba':
            loss_train,_ = train(model, device, train_loader, optimizer, epoch+1)
            loss_train1,_ = train(model, device, train_loader1, optimizer, epoch+1)
            loss_train = (loss_train * 19576 + loss_train1 * 133) / (19576+133)
            loss_train_list.append(loss_train)
        else:
            loss_train = train(model, device, train_loader, optimizer, epoch+1)
            loss_train_list.append(loss_train)
        if dataset=='kiba':
            G,P = predicting(model, device, valid_loader)
            G1,P1 = predicting(model, device, valid_loader1)
            G = torch.cat((G, G1), 0)
            P = torch.cat((P, P1), 0)
        else:
            G,P = predicting(model, device, valid_loader)
        G = G.numpy().flatten()
        P = P.numpy().flatten()
        ret_val = [mse(G,P), get_rm2(G,P)]
        loss_test_list.append(ret_val[0])
        if ret_val[0]<best_mse:
            best_mse = ret_val[0]
            best_r2 = ret_val[1]
            ...
            G_ = G
            P_ = P
            print('rmse improved at epoch ', best_epoch, '; best_mse, rm2:', ret[0], ret[1] ,dataset)
        else:
            print(best_mse,'No improvement since epoch ', best_epoch, '; best_mse, rm2:', ret[0], ret[1], dataset)
        time_end = time.time()
