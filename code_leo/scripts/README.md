...
14 组 (n1, n2)：`(4,2) (4,3) (5,2) (5,3) (5,4) (6,2) (6,3) (6,4) (7,2) (7,3) (7,4) (8,2) (8,3) (8,4)`

其中 (6,3) 为当前默认值。

如果已跑过 11 组，补跑 3 组缺失的用 `supplement_n1n2.sh`（6 个实验）。
...
| n1/n2 网格 | 28 | 2 数据集 x 14 组, 1 fold x 1 seed |
| MMD 敏感性 | 6 | 1 数据集 × 6 组, 1 fold × 1 种子 |
| **总计** | **433** | |
...
```
Step 1 ─ 敏感性分析（消融实验，1 fold x 1 seed，快速验证）
    ├── sensitivity_mmd.sh              # MMD beta 系数（Davis，5 组）
    ├── sensitivity_n1n2.sh             # n1/n2 网格搜索（Davis，14 组）
    ├── sensitivity_n1n2_human.sh       # n1/n2 网格搜索（Human，14 组）
    └── supplement_n1n2.sh             # 补充缺失网格组（如已跑过11组）
...
- Human/C.elegans 实验前需先删除旧的 `.pt` 文件并重新运行 `create_data_Human_Celegans.py`
