#!/usr/bin/env python3
"""
statistical_tests.py — HAG-DTA vs baseline 统计显著性检验 (paired t-test)

读取 aggregate_results.py 汇总的 per-fold results，与论文 baseline 做 t-test。
审稿人 R1 和 R2 明确要求。
...
# C.elegans (Precision↑, Recall↑, AUROC↑)
CELEGANS_BASELINES = {
...
def load_hag_results(csv_dir, dataset, model='Diff_DTA_GIN'):
    """从 per-fold random.csv 读取 HAG-DTA 5-fold 结果，返回 per-fold 均值列表"""
    values = {}
    for f in sorted(csv_dir.glob(f'{dataset}_{model}_fold*_random.csv')):
        df = pd.read_csv(f)
        for col in df.columns:
            if col not in values:
                values[col] = []
            values[col].extend(df[col].tolist())
    # 按 fold 聚合均值
    fold_means = {}
    for f in sorted(csv_dir.glob(f'{dataset}_{model}_fold*_random.csv')):
        df = pd.read_csv(f)
        fold_id = f.stem.replace(f'{dataset}_{model}_', '').replace('_random', '')
        fold_means[fold_id] = df.mean()
    return fold_means, values


def welch_t_test(hag_values, baseline_mean, baseline_std, metric, better='higher'):
    """Welch t-test between HAG results and baseline"""
    ...
    if baseline_std is None:
        # 单样本 t-test (HAG vs 固定值)
        t_stat, p_value = stats.ttest_1samp(hag_values, baseline_mean)
    else:
        # Welch's t-test (两独立样本，不等方差)
        se = np.sqrt(hag_std**2 / n + baseline_std**2 / 5)  # assume baseline n=5
        t_stat = (hag_mean - baseline_mean) / se
        df_num = (hag_std**2 / n + baseline_std**2 / 5)**2
        df_den = ((hag_std**2 / n)**2 / (n-1) + (baseline_std**2 / 5)**2 / 4)
        df = df_num / df_den
        p_value = 2 * stats.t.sf(abs(t_stat), df)

    # 对于 MSE (lower is better)，反转 t 符号
    if better == 'lower':
        t_stat = -t_stat
...
def main():
    ...
    for ds, metric, better, baselines in configs:
        fold_means, all_vals = load_hag_results(csv_dir, ds)
        if metric not in all_vals or not all_vals[metric]:
            print(f'{ds} / {metric}: no HAG results yet')
            continue

        hag_arr = np.array(all_vals[metric])

        print(f'--- {ds} — {metric} ({"↑" if better=="higher" else "↓"}) ---')
        ...
        for name, vals in baselines.items():
            bm, bstd = vals.get(metric.lower(), (None, None))
            if bm is None:
                continue
            result = welch_t_test(hag_arr, bm, bstd, metric, better)
            if result is None:
                continue
            sig = '✅ YES' if result['p_value'] < 0.05 else (
                  '⚠️  p<0.1' if result['p_value'] < 0.10 else '   no')
            baseline_str = f'{bm:.4f}' + (f'±{bstd:.4f}' if bstd else '')
